...
experiments in gh-run-receptor when the target condition is safe and cheap to generate,
and the initial non-UIBCDF corpus in `devtools/public_runs.toml`. There is no separate
fixture repository yet. Live tests are manual by default to avoid consuming API quota and
runner time on every commit. They verify the real GitHub adapter and are not the sole
coverage for any semantic rule.

`python devtools/scripts/validate_public_runs.py` independently compares every external
generic receptor report with `gh run view`, enforces bounded one-line LLM output, and
requires deterministic offline replay for a nontrivial matrix. Exit 2 means the external
evidence could not be acquired; exit 1 means acquired evidence violated the contract. Do
not place this gate in deterministic pull-request CI, and do not replace an expired run
without documenting equivalent workflow-shape coverage in its issue-backed report.

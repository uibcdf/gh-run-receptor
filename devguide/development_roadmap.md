...
| Phase 1: generic CLI | 30% | 29% | Capture, strict validated replay, nine registered serialized boundaries with release-specific freezes, forward-only migration rules, source-referenced normalization, bounded reports, attempt-specific historical truth, a hosted 0.21.1 candidate for the non-overlapping CLI exit-code map, degraded log analysis, and structured redacted acquisition failures are tested; patch distribution, remaining real outcome cases, and byte-identical replay timing remain |

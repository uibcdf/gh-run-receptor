...
| Phase 1: generic CLI | 30% | 29% | Capture, strict validated replay, eight registered and frozen serialized boundaries, forward-only migration rules, source-referenced normalization, bounded reports, attempt-specific historical truth, outcome exit-code parity, degraded log analysis, and structured redacted acquisition failures are tested; remaining real outcome cases and byte-identical replay timing remain |
...
| Phase 4: aggregation and comparison | 10% | 9% | Transition-only watch, versioned offline/remote comparison, and explicit regression policies have hosted evidence; structured producer-event aggregation is frozen in 0.20.0; provisional bounded multi-run aggregation passes offline truth tables and one live cross-repository pilot but still needs an exact-revision hosted gate before 0.21.0 |
| Cross-cutting release gate | 10% | 10% | Python packaging passes the nine OS/interpreter combinations; released wheel installation, script-extension installation, remote-source Action execution, terminal downstream reporting, and published-report consumption pass; the checksum-pinned GitHub CLI 2.48.0 minimum and uninterrupted draft-first 0.20.0 publication pass hosted gates, with all eight registered contracts frozen against published history |
